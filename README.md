...
### Upgrade on a schedule (automated with Dependabot)

Pinned versions only stay healthy if you refresh them. Rather than remembering to
run `pip-compile -U` by hand, **Dependabot** does it for us on GitHub (config:
`.github/dependabot.yml`). Each week it checks for newer versions and, if any
exist, opens a pull request:

- **`pip`**: because `requirements.txt` is generated by pip-compile, Dependabot
  edits `requirements.in` and **re-compiles** `requirements.txt`, keeping the two
  in sync exactly as you would by hand. Updates are **grouped** into a single PR
  to keep the noise down.
- **`github-actions`**: bumps the action versions pinned in
  `.github/workflows/*.yml` (e.g. `actions/checkout`, `actions/setup-python`).

CI (pre-commit + the test suite) runs on every Dependabot PR, so an upgrade only
gets merged once it's gone green; you review the diff and click merge.

Two things Dependabot does **not** manage, so handle them deliberately:

- **pre-commit hooks** (`.pre-commit-config.yaml`): there's no Dependabot
  ecosystem for these; keep running `pre-commit autoupdate`.
- **Big components** (Python itself, or a future web framework): upgrade these as
  separate, considered changes rather than rolling them into routine bumps.

You can still run the manual commands above any time; Dependabot just means you
rarely need to.
